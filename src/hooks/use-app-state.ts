  ...
  useEffect(() => {
    const loadState = async () => {
      try {
        ...
        if (savedState && savedState.searchQueries && savedState.searchQueries.length > 0) {
          // Load saved state if it exists and has data
          ...
          // No saved data, automatically load demo data
